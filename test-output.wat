(module
	...
	(func $test/test-sub-wat
		(table.get $wat4wasm (i32.const 11));; anoth ... me...
	...
	(func $main
		...
		(table.get $wat4wasm (i32.const 10));; özgür

		(call $self.console.log<ext>)
		(table.get $wat4wasm (i32.const 4)) ;; $self.Worker<ext>

		(call $self.console.log<ext>)

		(call $self.Reflect.construct<ext.ext>ext
			(table.get $wat4wasm (i32.const 3)) ;; $self.Worker<ext>ext<ext>

			(call $self.Array.of<ext>ext
				(table.get $wat4wasm (i32.const 9));; worker.js
			)
		)
		...
		(call $self.Reflect.construct<ext.ext>ext
			(table.get $wat4wasm (i32.const 2)) ;; $self.Number<ext>
		...
		(call $self.Reflect.construct<ext.ext>ext
			(table.get $wat4wasm (i32.const 1)) ;; $self.Object<ext>
	...
	(elem $wat4wasm declare func $wat4wasm $elemreq)
	(func $wat4wasm
		...
		(block $oninit
			...
			(block $decodeText/14:6
				...
				(table.set $wat4wasm (i32.const 7)
					(call $self.Reflect.apply<ext.ext.ext>ext
						(local.get $textDecoder.decode)
						(local.get $textDecoder)
						(local.get $arguments) ;; Number
				))
			)

			(block $decodeText/20:6
			...
			(block $decodeText/42:15

				(local.set $viewAt (i32.const 0))
				(local.set $offset (i32.const 42))
		...
		(block $ontextready
			...
			(block $self.Object<ext>.ref
				...
				(table.set $wat4wasm (i32.const 1)
					(call $self.Reflect.get<ext.ext>ext
						(global.get $self)
						(table.get $wat4wasm (i32.const 8));; Object
						;; Object
					)
			))

			(block $self.Number<ext>.ref

				(table.set $wat4wasm (i32.const 2)
					(call $self.Reflect.get<ext.ext>ext
						(global.get $self)
						(table.get $wat4wasm (i32.const 7));; Number
						;; Number
					)
			))
			...
			(block $self.Worker<ext>ext<ext>.ref
				...
				(table.set $wat4wasm (i32.const 3)
					(call $self.Reflect.get<ext.ext>ext
						(global.get $self)
						(table.get $wat4wasm (i32.const 5));; Worker
						;; Worker
					)
			))

			(block $self.Worker<ext>.ref
	...
	(data $wat4wasm "\30\30\30\30\30\30\30\30\57\6f\72\6b\65\72\4e\75\6d\62\65\72\4f\62\6a\65\63\74\77\6f\72\6b\65\72\2e\6a\73\c3\b6\7a\67\c3\bc\72\61\6e\6f\74\68\65\72\20\74\69\6d\65\2e\2e\2e")
	(start $wat4wasm)
)
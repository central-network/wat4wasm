...
const helpers = {
    ...
    nameSignatureofGlobal($name) {
        return String($name || '').match(/\$(.[^<]*)(?:\<(.[^>]*)\>)?/)?.slice(1) || [];
    },
    ...
    lastBlockOf(raw, keyword, filter) {
        ...
        if (!block) throw new Error(`no block: ${keyword} <-> ${block} // ${raw}`);

        if (block.toString().trim().startsWith('(module'))
            throw new Error(`block parsing failed: ${keyword} `);

        if (!block.toString().trim().startsWith(keyword))
            throw new Error(`block parsing failed: ${keyword} <-> ${block}`);

...
export default function (wat) {
    return wat
        ...
        .replaceAll("(void)", "(call $self.Function<>ext)")
        ...
        .replaceAll("(isNaN ", "(call $self.isNaN<ext>i32 ")
        .replaceAll("(bool ", "(call $self.Boolean<ext>i32 ")
        ...
        .replaceAll(/\(arguments(\s+)(\d+)\)/g, `(local.get $1)`)
        .replaceAll(/\((log|warn|debug|error|groupEnd|group)(\s*)\"(.[^\"]*?)\"\s*\)/g, `(call $self.console.$1<ext>$2(text "$3"))`)
        .replaceAll(/\((log|warn|debug|error)(\s+)/g, `(call $self.console.$1<ext>$2`)

...
export default function (wat, WAT4WASM) {
    ...
    while (helpers.hasBlock(wat, BLOCK_NAME)) {
        ...
        if (wrapper.$name.startsWith("$self") === false) {
            wrapper.$name = `$self.${wrapper.name}`;
        }

        if (wrapper.$name.endsWith(">") === false) {
            if (wrapper.descriptorKey === "value") {
                wrapper.$name = `${wrapper.$name}<ext>`;
            }
        }

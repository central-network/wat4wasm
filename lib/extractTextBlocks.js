    ...
    return watSource.replaceAll(textRegex, (match, rawContent) => {
        ...
        if (!entry) {
            ...
            InjectManager.addInstruction(
                TableManager.generateSetter(tableIndex, decodeCall, true)
            );

            entry = { rawContent, offset, length, tableIndex, buffer };
...
export function generateTextSections() {
    ...
    InjectManager.addLocal("$__decode_func__");
    InjectManager.addLocal("$__new_decoder__");
    InjectManager.addLocal("$__decode_args__");
    InjectManager.addLocal("$__newviewargs__");
    InjectManager.addLocal("$self.Uint8Array");

    ScopeManager.add(`(data $wat4wasm "\\${hexData}")`);
}

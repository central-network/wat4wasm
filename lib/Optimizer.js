...
export function optimizeWat(sourceCode, extraLocals = "", prepareCode = "") {
    ...
    selfGroups.forEach((groupBlocks, rootName) => {
        ...
        groupBlocks.filter((b, i, t) => {
            return 1 || t.findIndex(d => d.name === b.name) === i;
        }).forEach((block, i, t) => {
            ...
            if (tableSets.has(oldName) || globalSets.has(oldName)) {

                const t_indices = tableSets.get(oldName) || [];
                const t_setters = t_indices.map(idx => {
                    const setter = `(table.set $wat4wasm (i32.const ${idx}) (local.get ${currentVar}))`;
                    newContent = newContent.replace(`(table.set $wat4wasm (i32.const ${idx}) (local.get ${oldName}))`, ``);
                    if (groupLines.some(l => l.includes(setter)) || newContent.includes(setter)) {
                        return "";
                    }
                    return setter;
                }).filter((s, i, t) => t.lastIndexOf(s) === i).filter(Boolean).join("\n");

                const g_indices = globalSets.get(oldName) || [];
                const g_setters = g_indices.map(idx => {
                    let setter = `(global.set ${oldName} (local.get ${currentVar}))`;
                    if (groupLines.some(l => l.includes(setter)) || newContent.includes(setter)) {
                        return "";
                    }
                    return setter;
        ...
        groupLines = groupLines.filter(g => g.trim());
        groupLines = groupLines.filter((g, i, t) => t.findLastIndex(d => d.replaceAll(/\s+/g, " ") === g.replaceAll(/\s+/g, " ")) === i);
